...
class CyberBridgeInstance:
	def __init__(self):
		...
		self.name = None
		self.scenario_json = {}
		self.scenario_json["Ego"]= {}
		self.scenario_json["Obstacles"] = {}
...
#   }
	...
	def receive_publish(self, response:bytes):
		...
		if channel == '/apollo/localization/pose':
			...
			temp["heading"] = heading
			with self.lock:
				self.scenario_json["Ego"][pose.header.timestamp_sec] = {}
				self.scenario_json["Ego"][pose.header.timestamp_sec] = temp
			# print(Pose.localization.position)
			...
			oblist = []
			for _i in PerceptionObstacles.perception_obstacle:
				oblist.append({})
				if hasattr(_i,'id'):
					theta = _i.id
					oblist[-1]["id"] = theta
				...
				if hasattr(_i,"sub_type_name"):
					sub_type_name = _i.sub_type_name
					oblist[-1]["subTypeName"] = sub_type_name
			# if self.scenario_json[.header.timestamp_sec] != {}:
			# 	self.scenario_json[PerceptionObstacles.header.timestamp_sec]["Obstacles"] = oblist
			# else:
			with self.lock:
				self.scenario_json["Obstacles"][PerceptionObstacles.header.timestamp_sec] = {}
				self.scenario_json["Obstacles"][PerceptionObstacles.header.timestamp_sec] = oblist

			# print(PerceptionObstacles.header.timestamp_sec)
			# with open('/home/lsm/SFTSG_NME/src/pERCEPTION.txt', 'a') as f:
			# 	f.write(str(PerceptionObstacles))
			# 	f.write('1smsmsmsmmssmsmsmsmsmmsmsmsms\n')
			...
			print(Planning)
			with open('/home/lsm/SFTSG_NME/src/planning.txt', 'a') as f:
				f.write(str(Planning))
				f.write('1smsmsmsmmssmsmsmsmsmmsmsmsms\n')
	...
	def register(self, sim, destination, AgentNames, mapname, groundtruth,directory_name,name):
		#####lsm
		#create duiyingde wenjianjia baocun data
	...
	def save_pose_perception_json(self):
		if not os.path.exists("/home/lsm/SFTSG_NME/src/approach/RQ/RQ2/no_fuse_report_runtimedata"):
			os.makedirs("/home/lsm/SFTSG_NME/src/approach/RQ/RQ2/no_fuse_report_runtimedata")
			with self.lock:
				with open(fr"/home/lsm/SFTSG_NME/src/approach/RQ/RQ2/no_fuse_report_runtimedata/{self.name}_scenario.json", 'a') as f:
